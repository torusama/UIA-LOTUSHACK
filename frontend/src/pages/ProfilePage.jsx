...
function tierLabel(tier) {
  if (tier === "safety") return { text: "Safety", color: "#15803d", bg: "#dcfce7" };
  if (tier === "match")  return { text: "Match",  color: "#92400e", bg: "#fef9c3" };
...
function EssayFullResult({ entry }) {
  // EssayReview render result thông qua prop onResult
  // Thay vào đó ta import trực tiếp EssayResult component
  // Vì EssayResult không export riêng, ta tự render lại từ full_result
  const result = entry.full_result;
  if (!result) return <p style={{ color: "#9ca3af" }}>Không có dữ liệu đầy đủ.</p>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ marginBottom: 8 }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 18 }}>📝 Essay — {entry.school}</h3>
        <div style={{ fontSize: 12, color: "#9ca3af" }}>{timeAgo(entry.createdAt)}</div>
      </div>

      {/* Summary banner */}
      <div style={{ background: "#111827", borderRadius: 12, padding: "20px 24px", display: "flex", gap: 20, alignItems: "center" }}>
        <div style={{ flexShrink: 0 }}>
          <div style={{ fontSize: 28, fontWeight: 800, color: ovTag.bar }}>{ovTag.text}</div>
        </div>
        <div style={{ width: 1, height: 40, background: "#2d3748", flexShrink: 0 }} />
        <p style={{ margin: 0, color: "#9ca3af", fontSize: 13, lineHeight: 1.7 }}>{result.summary}</p>
      </div>

      {/* Scores */}
      <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
        <div style={{ padding: "10px 18px", borderBottom: "1px solid #f3f4f6", fontSize: 10, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: "#9ca3af" }}>
          Score Breakdown
        </div>
        {Object.entries(scores).filter(([k]) => k !== "overall").map(([k, v]) => {
          const tag = scoreTag(v);
          return (
            <div key={k} style={{ display: "flex", alignItems: "center", gap: 14, padding: "11px 18px", borderBottom: "1px solid #f9fafb" }}>
              <div style={{ width: 130, fontSize: 13, color: "#374151", flexShrink: 0 }}>{SCORE_LABELS[k] || k.replace(/_/g, " ")}</div>
              <div style={{ flex: 1, height: 5, background: "#f3f4f6", borderRadius: 99, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${(v / 10) * 100}%`, background: tag.bar, borderRadius: 99 }} />
              </div>
              <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 9px", borderRadius: 99, color: tag.color, background: tag.bg }}>{tag.text}</span>
            </div>
          );
        })}
      </div>
      ...
      {(result.strengths?.length > 0 || result.weaknesses?.length > 0) && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {[
            ...
            <div key={title} style={{ border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
              <div style={{ padding: "10px 18px", borderBottom: "1px solid #f3f4f6", fontSize: 10, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: hc }}>{title}</div>
          ...
          <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 10 }}>
            {suggestions.map((s, i) => (
              <div key={i} style={{ background: "#fffbeb", borderRadius: 10, padding: 14, fontSize: 13, borderLeft: "4px solid #f59e0b" }}>
                <div style={{ fontStyle: "italic", color: "#92400e", marginBottom: 8, padding: "4px 10px", background: "#fef3c7", borderRadius: 6 }}>"{s.quote}"</div>
      ...
      {criteria.length > 0 && (
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
          ...
          {criteria.map((c, i) => {
            ...
            return (
              <div key={i} style={{ padding: "13px 18px", borderBottom: "1px solid #f9fafb" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: "#111827" }}>{c.criterion?.replace(/_/g, " ")}</span>
                  <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 9px", borderRadius: 99, color: tag.color, background: tag.bg }}>{tag.text}</span>
                </div>
...
function InterviewFullResult({ entry }) {
  ...
  if (!report) return <p style={{ color: "#9ca3af" }}>Không có dữ liệu đầy đủ.</p>;
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 18 }}>🎤 Interview — {entry.school}</h3>
        <div style={{ fontSize: 12, color: "#9ca3af" }}>{timeAgo(entry.createdAt)}</div>
      </div>
      ...
      <div style={{ background: "#f0f9ff", borderRadius: 12, padding: 20, marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 16, marginBottom: 16, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontSize: 52, fontWeight: 700, color: "#1e40af", lineHeight: 1 }}>{report.overall_score}</div>
            <div style={{ fontSize: 12, color: "#6b7280" }}>Overall score</div>
          </div>
          {report.admission_likelihood_percent != null && (
            <div>
              <div style={{ fontSize: 36, fontWeight: 700, color: "#7c3aed", lineHeight: 1 }}>{report.admission_likelihood_percent}%</div>
        ...
        {Object.keys(dims).length > 0 && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
            {Object.entries(dims).map(([k, v]) => {
              ...
              return (
                <div key={k} style={{ background: t.bg, borderRadius: 8, padding: "6px 12px", textAlign: "center", minWidth: 80 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: t.color }}>{t.text}</div>
                  <div style={{ fontSize: 10, color: "#6b7280", marginTop: 2, textTransform: "capitalize" }}>{k.replace(/_/g, " ")}</div>
                </div>
              );
            })}
          </div>
        ...
        {report.summary && <p style={{ color: "#374151", fontSize: 14, lineHeight: 1.7, margin: 0 }}>{report.summary}</p>}
      </div>

      {report.top_moments?.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 8 }}>✨ Highlights</div>
          {report.top_moments.map((m, i) => (
            <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6, fontSize: 14, color: "#15803d" }}>
              <span>•</span>{m}
            </div>
          ))}
        </div>
      ...
      {report.improvement_tips?.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 8 }}>📌 Areas to improve</div>
      ...
      {report.next_steps?.length > 0 && (
        <div>
          <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 8 }}>🚀 Next steps</div>
...
function DashboardFullResult({ entry }) {
  ...
  if (!data) return <p style={{ color: "#9ca3af" }}>Không có dữ liệu đầy đủ.</p>;
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 18 }}>📊 Dashboard — {entry.school}</h3>
        <div style={{ fontSize: 12, color: "#9ca3af" }}>{timeAgo(entry.createdAt)}</div>
      </div>

      <div style={{ display: "flex", alignItems: "flex-end", gap: 20, marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 56, fontWeight: 700, color: "#1e40af", lineHeight: 1 }}>{data.estimated_probability}%</div>
          <div style={{ color: "#6b7280", fontSize: 13 }}>Xác suất trúng tuyển (Acceptance rate: {data.acceptance_rate}%)</div>
        </div>
        <span style={{ background: tc.bg, color: tc.text, padding: "6px 18px", borderRadius: 20, fontWeight: 700, fontSize: 14, marginBottom: 6 }}>
      ...
      {data.breakdown && (
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 12 }}>Điểm từng tiêu chí</div>
            ...
            <div key={name} style={{ marginBottom: 10 }}>
              ...
              <div style={{ height: 6, background: "#f3f4f6", borderRadius: 99, overflow: "hidden" }}>
                <div style={{
                  ...
                  width: `${info.score}%`,
                  background: info.score >= 75 ? "#10b981" : info.score >= 50 ? "#f59e0b" : "#ef4444",
                }} />
              </div>
            </div>
          ))}
...
function HistoryCard({ entry, onClick }) {
  ...
  const iconBgs = { essay: "#eff6ff", interview: "#f0fdf4", dashboard: "#fdf4ff" };
  const titles = {
  ...
  return (
    <div
      ...
      style={{
        ...
        marginBottom: 10, cursor: "pointer", transition: "box-shadow 0.15s, transform 0.1s",
      }}
      onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 4px 16px rgba(0,0,0,0.08)"; e.currentTarget.style.transform = "translateY(-1px)"; }}
      onMouseLeave={e => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.transform = "none"; }}
    >
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", background: "white" }}>
        <div style={{
          width: 44, height: 44, borderRadius: 10, background: iconBgs[entry.type],
          display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 20,
        }}>{icons[entry.type]}</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: 14, color: "#111827" }}>{titles[entry.type]}</div>
          <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>{timeAgo(entry.createdAt)}</div>
        </div>
        {tag && (
          <span style={{
        ...
        <span style={{ color: "#9ca3af", fontSize: 11, flexShrink: 0, marginLeft: 4 }}>→</span>
      </div>
    </div>
  );
}
...
export default function ProfilePage({ user }) {
  ...
  const [viewing, setViewing]   = useState(null); // entry đang xem
  ...
  return (
    <div>
      {/* Modal xem lại kết quả */}
      {viewing && (
        <Modal onClose={() => setViewing(null)}>
          {viewing.type === "essay"     && <EssayFullResult     entry={viewing} />}
          {viewing.type === "interview" && <InterviewFullResult entry={viewing} />}
          {viewing.type === "dashboard" && <DashboardFullResult entry={viewing} />}
        </Modal>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 28 }}>
        ...
        <div>
          <h2 style={{ margin: "0 0 4px", fontSize: 22, color: "#111827" }}>{user.name}</h2>
          <div style={{ fontSize: 14, color: "#6b7280" }}>{user.email}</div>
          {user.createdAt && (
            <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 4 }}>
              Tham gia:{" "}
              {(user.createdAt?.toDate ? user.createdAt.toDate() : new Date(user.createdAt)).toLocaleDateString("vi-VN")}
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 28 }}>
        {[
          { label: "Lần phân tích Essay",    value: counts.essay,     icon: "📝", color: "#eff6ff", textColor: "#1d4ed8" },
          { label: "Lần phỏng vấn",           value: counts.interview, icon: "🎤", color: "#f0fdf4", textColor: "#15803d" },
          { label: "Lần phân tích Dashboard", value: counts.dashboard, icon: "📊", color: "#fdf4ff", textColor: "#7c3aed" },
        ].map(({ label, value, icon, color, textColor }) => (
          <div key={label} style={{ background: color, borderRadius: 12, padding: "16px 18px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <h3 style={{ margin: 0, fontSize: 16, color: "#111827" }}>Lịch sử hoạt động</h3>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: "40px 0", color: "#9ca3af", fontSize: 14 }}>Đang tải lịch sử...</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "48px 24px", border: `2px dashed ${T.border}`, borderRadius: 14, color: "#9ca3af", fontSize: 14 }}>
          <div style={{ fontSize: 32, marginBottom: 10 }}>📭</div>
          Chưa có hoạt động nào{filter !== "all" ? " loại này" : ""}.
        </div>
      ) : (
        <>
          <p style={{ fontSize: 12, color: "#9ca3af", marginBottom: 12 }}>Click vào bất kỳ mục nào để xem lại kết quả đầy đủ</p>
          {filtered.map(entry => (
            <HistoryCard key={entry.id} entry={entry} onClick={() => setViewing(entry)} />
          ))}
        </>
      )}
    </div>
